...
function pegarMenorFaturamento(dado){
    ...
    dado.forEach(element => element.valor > smallestNumber ? smallestNumber = element.valor : "nothing");
    dado.forEach(element => {
        if (element.valor < smallestNumber && element.valor != 0) {
            smallestNumber = element.valor;
        }
    });
    console.log(`Valor com menor faturamento que não seja 0: ${smallestNumber.toFixed(2)}`);
}

function pegarMaiorFaturamento(dado){
    let conta = 0;
    dado.forEach(element => element.valor > conta ? conta = element.valor : "nothing");
    console.log(`Valor com maior faturamento: ${conta.toFixed(2)}`);
}
